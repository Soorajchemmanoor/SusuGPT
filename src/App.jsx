...
import { Send, User, Bot, Loader2, ChevronDown, Check, Copy, CheckCircle2, Paperclip, X, Plus, Trash2, PanelLeft, Users, Menu, Mic, MicOff, Volume2 } from 'lucide-react';
...
export default function App() {
  ...
  const speakMessage = (text, messageId) => {
    ...
    const googleVoice = voices.find(v => v.name.includes('Google UK English Female') || v.name.includes('Google US English'));
                  ...
                  <div className={cn(
                    ...
                    {msg.role === 'assistant' && msg.modelName && (
                      <div className="mt-1 flex items-center justify-between">
                        <div className="text-[9px] leading-[5px] font-black text-zinc-400 dark:text-zinc-500 flex items-center gap-1.5 uppercase tracking-widest select-none opacity-80">
                        ...
                        <button
                          ...
                          className={cn(
                            "p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors",
                            speakingMessageId === i ? "text-blue-500 bg-blue-50 dark:bg-blue-900/20" : "text-zinc-400"
                          )}
                          title="Read aloud"
                        >
                          <Volume2 size={14} className={cn(speakingMessageId === i && "animate-pulse")} />
                        </button>
                      </div>
                    )}
                  </div>
        ...
        <footer className="w-full max-w-3xl mx-auto p-4 pb-12 lg:pb-8 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="relative bg-[#f4f4f4] dark:bg-[#2f2f2f] rounded-[26px] p-1.5 lg:p-2 pr-3 flex items-end shadow-sm border border-zinc-200/50 dark:border-zinc-700/50 group focus-within:ring-1 focus-within:ring-zinc-300 dark:focus-within:ring-zinc-600 transition-all">
            ...
            <button
              ...
              className={cn(
                "p-2 mb-1.5 text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-full transition-all shrink-0 relative",
            ...
            >
              ...
              {isListening && (
                <span className="absolute inset-0 rounded-full animate-ping-slow bg-red-500 opacity-20" />
              )}
            </button>
            <textarea
              value={input}
